...
Scenario Outline: User runs psm-convert
  ...
  And I pass the arguments --overwrite T <extra_args> test_results/<results> <out_format>
  ...
  When I ignore lines matching the pattern: /^<search_database .*$/
...
Examples:
  |test_name           |results         |out_format|actual_output      |expected_output          |extra_args                          |
  ...
  |psmconv-txt-to-pep1 |results1.tide-search.txt    |pepxml    |psm-convert.pep.xml|psmconv-from-txt1.pep.xml|--protein-database small-yeast.fasta|
  |psmconv-txt-to-pep2 |results2.tide-search.txt    |pepxml    |psm-convert.pep.xml|psmconv-from-txt2.pep.xml|--protein-database small-yeast.fasta|
