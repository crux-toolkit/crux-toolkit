...
#define MAX_VARMOD_AA \
    32  // max # of modified AAs in a peptide per variable modification
...
struct CometScores {
    ...
    CometScores(double xCorr, double dSp, double dCn, double dExpect,
           double dAScorePro, double mass, int matchedIons, int totalIons,
           string sAScoreProSiteScores)
        : xCorr(xCorr),
          dSp(dSp),
          dCn(dCn),
          dExpect(dExpect),
    ...
    CometScores(CometScores&& a) noexcept
        : xCorr(a.xCorr),
          dSp(a.dSp),
          dCn(a.dCn),
          dExpect(a.dExpect),
          ...
          matchedIons(a.matchedIons),
          totalIons(a.totalIons),
          sAScoreProSiteScores(std::move(a.sAScoreProSiteScores)) {}

    CometScores& operator=(CometScores&& a) noexcept {
        xCorr = a.xCorr;
        dSp = a.dSp;
        dCn = a.dCn;
        dExpect = a.dExpect;
    ...
    ~CometScores() {
        std::cout << "Destroying CometScores object" << std::endl;
        try {
            // code
        } catch (...) {
            std::cout << "Exception during CometScores destruction" << std::endl;
...
struct ScoresMS1 {
    ...
    ScoresMS1() : fDotProduct(0), fRTime(0), iScanNumber(0) {}

    ScoresMS1(float fDotProduct, float fRTime, int iScanNumber)
        : fDotProduct(fDotProduct), fRTime(fRTime), iScanNumber(iScanNumber) {}
...
struct Fragment {
    ...
    Fragment(double mass, double intensity, int type, int number, int charge,
             bool neutralLoss, double neutralLossMass)
        : mass(mass),
          intensity(intensity),
          type(type),
          number(number),
    ...
    double ToMz() { return (mass + (charge - 1.0) * 1.00727646688) / charge; }
};
...
struct VarMods {
    ...
    int iWhichTerm;
    int iRequireThisMod;  // 0=no; 1=required; negative number = different
                          // functionality allowing only one from a set of mods
...
struct EnzymeInfo {
    bool bNoEnzymeSelected;   // set to true if enzyme is no-enzyme (cut
                              // everwhere)
    bool bNoEnzyme2Selected;  // set to true if 2nd enzyme is no-enzyme (cut
                              // everywhere)
    ...
    int iSearchEnzyme2OffSet;  // 2nd enzyme; this will be set to -9 if second
                               // enzyme is not used
...
enum HasVariableModType {
    ...
    HasVariableModType_AScorePro  // peptide contains a variable modification
                                  // that will get localized with AScorePro
};

struct InputFileInfo {
    int iInputType;
    int iAnalysisType;
    int iFirstScan;  // for scan range specified with file on command line;
...
class TypedCometParam : public CometParam {
   public:
    TypedCometParam(CometParamType paramType, const string& strValue,
                    const T& value)
