...
identification.
\end{abstract}
% FIXME possibly include ``open source'' nature in abstract
% FIXME possibly include turbosequest in abstract
...
\item {\bf Efficient retrieval of candidate peptides.}  Given a
  ...
  % FIXME mention turbosequest, but only on windows
...
\item {\bf On-the-fly generation of decoy peptides.}  In evaluating
  ...
  % FIXME mention tandem also has on the fly decoy generation 
  % but only reversed sequences
...
% FIXME inversion of Sequest and Crux w/o index across the two operating
% systems is strange, but explicable. Different OS and different sequest
% executables. Maybe add explanation to discussion or results? 
...
supplement.
% FIXME put in url for supplement, but where?
...
Sequest} on this OS.
% FIXME put in turbosequest results
