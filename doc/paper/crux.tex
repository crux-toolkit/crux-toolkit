...
candidate peptides, whose masses lie within a specified range of the
...
space-efficient but can be slow.

Crux implements the above approach, but it also offers an alternative
...
Crux stores the binary protein database and the peptide index as
...
ensuring that small databases are kept in memory and large databases
get written to disk.  To facilitate retaining frequently accessed
regions of the peptide index, the index is implemented as a collection
of memory-mapped files, each corresonding to a small range of peptide
masses.
