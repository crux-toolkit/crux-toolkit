...
protein sequence from the {\em Saccharomyces cerevisiae} (yeast) and 
{\em Homo sapiens} (human) predicted protein sequences, generating candidate peptides from a series of increasingly wider
...
possible charge state).  For SEQUEST and for Crux with a static
...
or peptide proteotypicity.  % probably not a word! % but is proteotypic? -AAK
...
while allowing missed cleavage sites for two protein sequence databases:
...
These indexing procedures required approximately four and two 
minutes of real time and produced indices of 69 and 240 MB, respectively.

We use a publicly available tandem mass spectrometry data
set from \citep{klammer:peptide2}, available as 60cm.ms2 at 
