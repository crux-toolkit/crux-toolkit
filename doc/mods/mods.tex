...
\caption{{\bf Modifying a Peptide Sequence} Inputs: (1) a sequence q
  containing an ordered list of amino acids, (2) a list M of peptide
  modifications.  The modification list consists of tuples, each with
  three parts: a unique mass shift (M[i].delta), a count of how many
  of those shifts will be applied to the sequence (M[i].count), and a
  set of amino acids to which this mass shift can be applied
  (M[i].aas).  For example, a peptide mod with M[0].count = 2 will
  apply a mass shift of M[0].delta to two different amino acids in the
  sequence, those amino acids belonging to the set M[i].aas.  Output:
  a set containing many copies of the given peptide, corresponding to
  all the ways that the given modification can be applied to the
  peptide.
  ...
  \Comment Apply each modification to the list of sequences.
    ...
    \Comment Apply modification specified number of times 
      ...
      \Comment Apply to each sequence
        \State Q += applyModToSequence(M[i].delta, M[i].aas, Q[j],
        j-1)
