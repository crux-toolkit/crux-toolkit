 ...
 * Default constructor.
 */
Spectrum::Spectrum() : first_scan_(0),
                       last_scan_(0),
                       precursor_mz_(0),
                       min_peak_mz_(0),
                       max_peak_mz_(0),
 ...
 */
bool Spectrum::parseMstoolkitSpectrum(MSToolkit::Spectrum* mst_spectrum,  ///< the input MSToolkit spectrum -in
...
bool Spectrum::parsePwizSpecInfo(
    ...
    if (have_precursor_mz) {
        precursor_mz_ = iso_window.cvParam(pzd::MS_isolation_window_target_m_z).valueAs<double>();
    }
    ...
    if (!have_precursor_mz) {
        precursor_mz_ = selected_ion_m_z;
    }
    ...
    if (ions[0].hasCVParam(pzd::MS_charge_state)) {
        ...
        if (zstates_.empty()) {
            if (ions[0].hasCVParam(pzd::MS_possible_charge_state)) {
                ...
                }  // add +1, +2, ... max-precursor-charge
                else {
                    charge_state_assigned_ = assignZState();
                }  // do choose charge and add +1 or +2,+3
            }
        }
    }
 ...
 */
void Spectrum::addPeak(FLOAT_T intensity,   ///< the intensity of peak to add -in
...
vector<SpectrumZState> Spectrum::getZStatesToSearch() {
    ...
    if (charge_str == "all") {  // return full array of charges
        ...
        if ((param_charge < 1) || (param_charge > MAX_CHARGE)) {
            carp(CARP_FATAL,
                 "spectrum-charge option must be 1,2,3,.. %d or 'all'.  "
                 "'%s' is not valid",
                 MAX_CHARGE, charge_str.c_str());
        }
