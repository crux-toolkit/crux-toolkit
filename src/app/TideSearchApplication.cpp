...
int TideSearchApplication::main(const vector<string>& input_files, const string input_index) {
  ...
  if (Params::GetBool("sqt-output") || Params::GetBool("pepxml-output") ||
    ...
    NUM_THREADS = 1;
    carp(CARP_INFO, "Threading for non tab-delimited file formats are not supported yet");
  } else if (Params::GetBool("peptide-centric-search") == true) {
    NUM_THREADS = 1;
  ...
  for (vector<InputFile>::const_iterator f = input_sr.begin();
    ...
    carp(CARP_INFO, "Elapsed time after conversion: %.3g s", wall_clock() / 1e6);
...
void TideSearchApplication::search(void* threadarg)
{
  ...
  long thread_num = my_data->thread_num;
  long num_threads = my_data->num_threads;
  int nAA = my_data->nAA;
  double* aaFreqN = my_data->aaFreqN;
  double* aaFreqI = my_data->aaFreqI;
  double* aaFreqC = my_data->aaFreqC;
  int* aaMass = my_data->aaMass;
  pthread_rwlock_t ** locks_array = my_data->locks_array;
  ...
  for (vector<SpectrumCollection::SpecCharge>::const_iterator sc = spec_charges->begin()+thread_num;
    ...
    if (spectrum_flag != NULL) {
      pthread_rwlock_wrlock(locks_array[1]);
      ...
      pthread_rwlock_unlock(locks_array[1]);
    }
    ...
    if (!exact_pval_search) {  //execute original tide-search program
      ...
      pthread_rwlock_wrlock(locks_array[2]);
      (*total_candidate_peptides) += nCandPeptide;
      pthread_rwlock_unlock(locks_array[2]);
    ...
    pthread_rwlock_wrlock(locks_array[3]);
    ++(*sc_index);
    ...
    pthread_rwlock_unlock(locks_array[3]);
  }
}
...
/**
  * This is the method called by search(spectrum_filename, spec_charge, ...) during
  * pthread_create for each thread. Casts threadarg as TideSearchApplication 
  * so that threading can be done and calls searchthread(void *threadarg).
  */
...
void TideSearchApplication::search(
  ...
  int num_locks = 4;
  pthread_rwlock_t * locks_array[num_locks];

  for (int i = 0; i < num_locks; i++) {
    locks_array[i] = new pthread_rwlock_t();
    pthread_rwlock_init(locks_array[i], NULL);
  }
  ...
  for (int i = 0; i < NUM_THREADS; i++) {
    active_peptide_queue[i]->SetOutputs(output_files, &locations, top_matches, compute_sp, target_file, decoy_file,highest_mz);
  }
  ...
  for (int i= 0; i < NUM_THREADS; i++) {
    thread_data_array[i] = thread_data(spectrum_filename, spec_charges, active_peptide_queue[i],
      ...
      highest_mz, output_files, target_file, decoy_file, compute_sp,
  ...
  carp(CARP_INFO, "Elapsed time before thread launch: %.3g s", wall_clock() / 1e6);

  // launch threads
  int rc;
  for (long t = 0; t < NUM_THREADS - 1; t++) {
    rc = pthread_create(&threads[t], NULL, &searchthread_helper, (void *) &(thread_data_array[t+1]));
  }
  ...
  carp(CARP_INFO, "Elapsed time after combined: %.3g s", wall_clock() / 1e6);
