    ...
    struct ChromatographicPeak
    {
        ...
        void calculateIntensityForThisFeature(bool integrate)
        {
            if (!isotopicEnvelopes.empty())
            {
                ...
                if (integrate)
                {
                    intensity = std::accumulate(isotopicEnvelopes.begin(), isotopicEnvelopes.end(), 0.0, [](double sum, const IsotopicEnvelope& p)
                                                { return sum + p.intensity; });
                }
                else
                {
        ...
        void mergeFeatureWith(ChromatographicPeak &otherFeature, bool integrate)
        {
            if (&otherFeature != this)
            {
                ...
                std::sort(mergedIdentifications.begin(), mergedIdentifications.end(), [](const Identification &a, const Identification &b)
                          { return a.posteriorErrorProbability < b.posteriorErrorProbability; });
