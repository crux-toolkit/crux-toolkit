...
CometResult::CometResult(Array<Tag*>* tags) : SearchResult(tags) {
  ...
  */
  if(processed_) {
	...
	if(! found[0] && ! strcmp(tag->getAttributeValue("name"), "xcorr")) {
	...
	else if(! found[5] && ! strcmp(tag->getAttributeValue("name"), "expect")) {
...
CometResult::CometResult(char* szBuf, Boolean preexisting_probs, int format) {
...
char* CometResult::extractDatabase(char* html) {
  char start[] = "&amp;Db=";
  char stop[] = "&amp";
...
void CometResult::process(char* szBuf, Boolean preexisting_probs, int format, int* formats, int numformats, char* valid) {
...
Boolean CometResult::parseUnknown(const char* szBuf, Boolean preexisting_probs, int* formats, int numformats) {
...
Boolean CometResult::parse(const char* szBuf, Boolean preexisting_probs, int format) {
...
const char* CometResult::getName() { return "Comet"; }
