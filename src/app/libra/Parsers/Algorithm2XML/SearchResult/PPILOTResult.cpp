#include "PPILOTResult.h"

/*

Program       : PPILOTResult for discr_calc of PeptideProphet 
...
PPILOTResult::PPILOTResult(char* szBuf, Boolean preexisting_probs){
  init();
...
PPILOTResult::PPILOTResult(Array<Tag*>* tags) : SearchResult(tags) {
  ...
  if(processed_) {
	...
	if(!found[0] && ! strcmp(tag->getAttributeValue("name"), "Paragon:score")) {
	  pScore_ = (double)(atof(tag->getAttributeValue("value")));
	  found[0] = True;
	}
...
/*
char* PPILOTResult::extractDatabase(char* html) {
  char start[] = "&amp;Db=";
  char stop[] = "&amp";
...
void PPILOTResult::process(char* szBuf, Boolean preexisting_probs, char* valid) {
}

const char* PPILOTResult::getName() {
  return "PPILOT";
}
