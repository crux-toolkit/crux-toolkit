#include "OMSSAResult.h"

/*

Program       : OMSSAResult for discr_calc of PeptideProphet 
...
OMSSAResult::OMSSAResult(char* szBuf, Boolean preexisting_probs){
  init();
...
OMSSAResult::OMSSAResult(Array<Tag*>* tags) : SearchResult(tags) {
  ...
  if(processed_) {
	...
	if(!found[0] && ! strcmp(tag->getAttributeValue("name"), "pvalue")) {
...
/*
char* OMSSAResult::extractDatabase(char* html) {
  char start[] = "&amp;Db=";
  char stop[] = "&amp";
...
void OMSSAResult::process(char* szBuf, Boolean preexisting_probs, char* valid) {
}

const char* OMSSAResult::getName() {
  return "OMSSA";
}
