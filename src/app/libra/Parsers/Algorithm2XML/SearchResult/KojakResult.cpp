#include "KojakResult.h"

/*

Program       : KojakResult for discr_calc of PeptideProphet 
...
KojakResult::KojakResult(char* szBuf, Boolean preexisting_probs){
  init();
...
KojakResult::KojakResult(Array<Tag*>* tags) : SearchResult(tags) {
  ...
  lower_score_ = 0;
  higher_score_ = 0;
  link_expect_lower_score_ = 0;
  link_expect_higher_score_ = 0;
  sum_rank_ = 0;
  ...
  if(processed_) {
	...
	if(!found[0] && ! strcmp(tag->getAttributeValue("name"), "kojak_score")) {
	  kojakScore_ = (double)(atof(tag->getAttributeValue("value")));
	  found[0] = True;
	}
	else if(!found[1] && ! strcmp(tag->getAttributeValue("name"), "delta_score")) {
	  deltaScore_ = (double)(atof(tag->getAttributeValue("value")));
	  found[1] = True;
	}
	else if(!found[2] && ! strcmp(tag->getAttributeValue("name"), "e-value")) {
	...
	if (higher_score_ < (double)(atof(tag->getAttributeValue("value")))) {
      ...
      if(! strcmp(tag->getName(), xlink_score_tag) && tag->isStart() && ! strcmp(tag->getAttributeValue("name"), "e-value")) {
	if (unset_e || link_expect_lower_score_  > (double)(atof(tag->getAttributeValue("value")))) {
	  link_expect_lower_score_ = (double)(atof(tag->getAttributeValue("value")));
	  lower_peptide_ = (*linkedPeps_)[linkedPeps_->size()-1];
	  unset_e = false;
	}
	if ( link_expect_higher_score_ < (double)(atof(tag->getAttributeValue("value")))) {
	   link_expect_higher_score_ = (double)(atof(tag->getAttributeValue("value")));
	  unset_e = false;
	}
      }   
  ...
  if (unset_e) {
    link_expect_lower_score_ = link_expect_higher_score_ = expectScore_;
  }

  if (linkedPeps_ &&
      linkedPeps_->size() == 2 &&
      ( (*linkedPeps_)[0].find((*linkedPeps_)[1]) != string::npos ||
	(*linkedPeps_)[1].find((*linkedPeps_)[0]) != string::npos ) ) {

      // (*linkedPeps_)[0]==(*linkedPeps_)[1]) {
    //    link_expect_lower_score_ = link_expect_higher_score_ = expectScore_;

    expectScore_ = link_expect_lower_score_ = link_expect_higher_score_;
...
/*
char* KojakResult::extractDatabase(char* html) {
  char start[] = "&amp;Db=";
  char stop[] = "&amp";
...
void KojakResult::process(char* szBuf, Boolean preexisting_probs, char* valid) {
}

const char* KojakResult::getName() {
  return "Kojak";
}
