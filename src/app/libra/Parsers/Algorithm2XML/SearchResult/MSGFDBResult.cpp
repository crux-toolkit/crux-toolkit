#include "MSGFDBResult.h"

/*

Program       : MSGFDBResult for discr_calc of PeptideProphet 
...
MSGFDBResult::MSGFDBResult(char* szBuf, Boolean preexisting_probs){
  init();
...
MSGFDBResult::MSGFDBResult(Array<Tag*>* tags) : SearchResult(tags) {
  ...
  if(processed_) {
	...
	if(!found[0] && ! strcmp(tag->getAttributeValue("name"), "P-value")) {
...
/*
char* MSGFDBResult::extractDatabase(char* html) {
  char start[] = "&amp;Db=";
  char stop[] = "&amp";
...
void MSGFDBResult::process(char* szBuf, Boolean preexisting_probs, char* valid) {
}

const char* MSGFDBResult::getName() {
  return "MSGFDB";
}
