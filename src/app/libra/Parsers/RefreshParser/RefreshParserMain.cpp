...
Date          : 11.27.02
SVN Info      : $Id: RefreshParserMain.cpp 7715 2017-12-20 02:59:14Z real_procopio $
...
int main(int argc, char** argv) {
  ...
  if(argc < 3) {
    ...
    cerr << " usage: RefreshParser <pepxmlfile> <database> (<min ntt>(defaults to 0,unless specified in pepXML file with min_number_termini in enzymatic_search_constraint) (DEGEN) (PROT_MW) (PREV_AA_LEN=<length(default=1)>) (NEXT_AA_LEN=<length(default=1)>) (RESTORE_NONEXISTENT_IF_PREFIX=str)" << endl;
#ifdef __LGPL__
  ...
  RefreshParser *parser = new RefreshParser(argv[1], argv[2], min_ntt, n_prev_aas, n_next_aas, degen, prot_wt, testArgs.c_str(), restore_prefix);
