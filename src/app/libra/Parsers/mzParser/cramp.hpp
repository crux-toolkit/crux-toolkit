...
public:
   ...
   /**
	...
	mzParser::ramp_fileoffset_t getScanOffset(size_t n) const {
...
private:
   ...
   mzParser::ramp_fileoffset_t *m_scanOffsets; // scan offset table
   int m_lastScan; // useful for cRampIterator
protected:
   friend class cRampIterator;
   rampInfo *do_ramp(mzParser::ramp_fileoffset_t arg, eWhatToRead whatToRead);
...
typedef struct {
  mzParser::RAMPREAL mz;
  mzParser::RAMPREAL intensity;
} rampPeakInfoStruct;

class rampPeakList : public rampInfo {
public:
  rampPeakList(mzParser::RAMPFILE *m_handle, mzParser::ramp_fileoffset_t index, int seqNum=-1); // populate from file at this position
private:
   int m_peaksCount;
...
public:
   ...
   //
   rampScanInfo(mzParser::RAMPFILE *m_handle, mzParser::ramp_fileoffset_t index, int seqNum); // populate from file at this position, assign this sequence number
   ...
   // data members
   //
   mzParser::ScanHeaderStruct m_data;
public:
...
public:
   ...
   // constructors, destructors
   //
   rampRunInfo(mzParser::RAMPFILE *m_handle); // populate from file

   rampRunInfo(const rampRunInfo &rhs) { // copy constructor
      memmove((void*)this,(void*)&rhs,sizeof(rhs));
      if (rhs.m_scanOffsets) { // need a deepcopy
         if (NULL!=(m_scanOffsets = (mzParser::ramp_fileoffset_t *)malloc(m_data.scanCount*sizeof(mzParser::ramp_fileoffset_t)))) {
            memmove(m_scanOffsets,rhs.m_scanOffsets,m_data.scanCount*sizeof(mzParser::ramp_fileoffset_t));
         }
      }
   };
   virtual ~rampRunInfo() {
   ...
   // data members
   //
   mzParser::RunHeaderStruct m_data;
   mzParser::ramp_fileoffset_t *m_scanOffsets;
};
...
class rampInstrumentInfo : public rampInfo {
  ...
  public:
    rampInstrumentInfo(mzParser::RAMPFILE *m_handle);
    rampInstrumentInfo(const rampInstrumentInfo &rhs) {
      m_instrumentStructPtr = 
      rhs.m_instrumentStructPtr?
      new mzParser::InstrumentStruct(*rhs.m_instrumentStructPtr) :
      NULL;
    }
    ...
    mzParser::InstrumentStruct* m_instrumentStructPtr;
