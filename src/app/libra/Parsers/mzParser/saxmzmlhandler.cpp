  ...
  if (isElement("binaryDataArray",el)){
    ...
    if (s.length() < 1) {
      ...
      if (it != m_mIndex.end()) {
        ...
        //cout << "WARNING: precursor:spectrumRef " << s << " is not indexed ... precursor scanNumber recorded as ZERO " << endl;
        m_precursorIon.scanNumber = 0;
      }
    }
...
void mzpSAXMzmlHandler::endElement(const XML_Char *el) {
  ...
  if(isElement("binary", el))  {
    ...
    if (m_precursorIon.isoLowerMZ <= 0) m_precursorIon.isoLowerMZ = m_precursorIon.isoMZ - m_precursorIon.isoLowerOffset;
    if (m_precursorIon.isoUpperMZ <= 0) m_precursorIon.isoUpperMZ = m_precursorIon.isoMZ + m_precursorIon.isoUpperOffset;
...
void mzpSAXMzmlHandler::processCVParam(const char* name, const char* accession, const char* value, const char* unitName, const char* unitAccession)
{
  if(!strcmp(name, "32-bit float") || !strcmp(accession,"MS:1000521"))  {
    ...
    if(m_precursorIon.monoMZ!=0) m_precursorIon.monoMZ=atof(value); //if the monoisotopic peak was specified earlier, this is a better value to use.
      ...
      m_precursorIon.monoMZ = m_precursorIon.mz; // DDS: Set it here otherwise it is ZERO
