   ...
   if (m_scanOffsets || (RAMP_RUNINFO == what) || (RAMP_INSTRUMENT == what)) {
      ...
      if (scanOffset >= 0) {
         ...
         case RAMP_HEADER:
            ...
            break;           
         case RAMP_PEAKS:
	   returnPtr = new rampPeakList( m_handle, scanOffset, (int)arg);
 ...
 **/
rampPeakList::rampPeakList(RAMPFILE *handle, ramp_fileoffset_t index, int seqNum) {
   init();
   //if (seqNum <= 0) { //not using seqNum information at this time
   m_peaksCount = readPeaksCount(handle,index);
   m_pPeaks = (rampPeakInfoStruct *)readPeaks(handle,index);
   //}
   //else {
   //  m_peaksCount = readPeaksCount(handle,index,seqNum-1);
   //  m_pPeaks = (rampPeakInfoStruct *)readPeaks(handle,index,seqNum-1);
   //}
}
