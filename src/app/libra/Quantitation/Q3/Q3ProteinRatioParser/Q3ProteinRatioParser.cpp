...
Date          : 11.27.02 
SVN info      : $Id: Q3ProteinRatioParser.cpp 8017 2020-02-12 18:34:06Z mhoopmann $
...
Q3ProteinRatioParser::Q3ProteinRatioParser(const char* protxmlfile, const char *testMode) : Parser("q3") { 
  searchHits_ = NULL;
  iNumRawData_=0;
  heavy2light_ = False;
...
Q3ProteinRatioParser::~Q3ProteinRatioParser() {
  delete searchHits_;
  free(testMode_);
}

void Q3ProteinRatioParser::parse(const char* protxmlfile) {
  ...
  if(! fin) {
    cout << "Q3ProteinRatioParser: error opening " << (protxmlfile == NULL ? "file" : protxmlfile) << std::endl;
    exit(1);
  }
  ...
  if (testType!=NO_TEST) {
    ...
    testFileName = constructTagListFilename(protxmlfile, // input file
					    ...
					    "Q3ProteinRatioParser", // program name
...
#define RECORD(tag) {(tag)->write(fout);if (testType!=NO_TEST) {test_tags.insertAtEnd(new Tag(*(tag)));}}
  ...
  double MIN_PEP_WT = 0.; //0.5;
  double MIN_PEP_PROB = 0.; //0.5
  Array<Tag*>* tags = NULL;
  peplist* peps = NULL;
  double next_prot_prob = 2.0;
  double MIN_PROB = 0.; //0.2
  Boolean done = False;
  ...
  Array<const char*>* inputfiles = new Array<const char*>;
  char** altpeps = NULL;
  ...
  while(fin.getline(nextline, line_width_)) {
	...
	if(tag->isStart() && ! strcmp(tag->getName(), "peptide")) {
	  ...
	  // add the asap pro
	  Tag* protag = NULL;
	  if(peps->size() > 0  && getRatio(peps, MIN_PEP_PROB)) {

	    char* peptidestring = getPeptideString(peps, "+");

	    protag = new Tag("Q3Ratio", True, True);
	    if(protag == NULL) {
	...
	if(tag->isEnd() && ! strcmp(tag->getName(), "protein_summary_header")) {
	  Tag* summary = new Tag("Q3_analysis_summary", True, True);
	  // get time info
	  char next[20];
  ...
  if(! overwrite(protxmlfile, outfile.c_str(), "</protein_summary>")) {
    cout << "error: no Q3 data written to file " << protxmlfile << endl;
  }
  ...
  if (testType!=NO_TEST) {
    ...
    // regression test stuff - bpratt Insilicos LLC, Nov 2005
    //
    TagListComparator("Q3ProteinRatioParser",testType,test_tags,testFileName);
    delete[] testFileName;
...
void Q3ProteinRatioParser::setFilter(Tag* tag) {
  if(tag == NULL)
    return;
...
void Q3ProteinRatioParser::enterUnique(peplist* uniques, const char* next) {
...
// must substitute # -> ~
char* Q3ProteinRatioParser::getPeptideString(peplist* peps, const char* link) {
...
Boolean Q3ProteinRatioParser::getRatio(peplist* peptides, double dProbability) { //char** peps, double minpepprob) {
  double mean = 0.0;
  double meansq = 0.0;
  int num = 0;
  ...
  parser_ = new Q3GroupPeptideParser(*searchHits_, peptides, dProbability);
...
// we'll ignore all but the interesting tags and attributes
static const char *Q3ProteinRatioParser_interesting_tags[] = {
  "search_hit",  "q3ratio_result",  "peptideprophet_result",   NULL
};

static const char *Q3ProteinRatioParser_interesting_attrs[] = {
  "hit_rank","peptide","heavy_area","light_area",
  "q2_heavy_area","q2_light_area","probability",NULL
};


void Q3ProteinRatioParser::cachePepXML() { 
  searchHits_ = new Q3RatioSearchHitCache;
  ...
  Q3RatioSearchHit hit;
  TagInclusionTest tagtest(Q3ProteinRatioParser_interesting_tags,
			   Q3ProteinRatioParser_interesting_attrs);
  ...
  for(int k = 0; k < input_pepxmlfiles_.length(); k++) {
	...
	if(tag && tag->getName()) {
	  ...
	  if(tag->isStart() && isSearchHitTag && ! strcmp(tag->getAttributeValue("hit_rank"), "1")) {
	    ...
	    if(tag->isStart() && ! strcmp(tag->getName(), "q3ratio_result")) {
	      hit.heavy_ = atof(tag->getAttributeValue("heavy_area"));
	      hit.light_ = atof(tag->getAttributeValue("light_area"));
	      // NOTE: these aren't actually used anywhere?
	      hit.q2_heavy_ = atof(tag->getAttributeValue("q2_heavy_area"));
	      hit.q2_light_ = atof(tag->getAttributeValue("q2_light_area"));
