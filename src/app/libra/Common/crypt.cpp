// LPGL notice: this file was altered by Insilicos 6-29-06 to create an 
// implementation of the crypt() commandline utility for win32.
...
void shuffle_sb(unsigned long *k, unsigned long saltbits)
  { int j, x;
    for(j=4096; j--;) {
      x = (k[0] ^ k[1]) & saltbits;
      *k++ ^= x;
...
char *output_conversion(unsigned long l1,unsigned long l2,unsigned long r1,unsigned long r2, char *salt)
...
int main(int argc, char **argv) {
  ...
  /* Random salt if none given. (First two char's of the encrypted string) */
  salt[0] = (argc>2)?(argv[1][0]):rndChar[rand() % 64];
  salt[1] = (argc>2)?(argv[1][1]):rndChar[rand() % 64];
  salt[2] = 0;

  puts(des_crypt(argv[(argc>2)?2:1],salt));
}
