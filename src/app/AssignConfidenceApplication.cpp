...
int AssignConfidenceApplication::main(const vector<string>& input_files) {
  ...
  if (!score_param.empty()) {
    ...
    case TAILOR_COL: //Added for tailor score calibration method by AKF
      ...
      break;
    case HYPER_SCORE_COL: //Added for tailor score calibration method by AKF
      score_type = HYPER_SCORE;      
      break;
    case HYPER_SCORE_TAILOR_COL: //Added for tailor score calibration method by AKF
      score_type = HYPER_SCORE_TAILOR;      
      break;
    case HYPER_POISSON_EVAL_COL: //Added for tailor score calibration method by AKF
...
string AssignConfidenceApplication::getDescription() const {
  return
    ...
    "4(3):207-14, 2007.</blockquote>"

    "<p>Note that <code>assign-confidence</code> "
    ...
    "adjusted accordingly (by dividing by 2).</p>"

    "<p>The alternative, <em>"
    ...
    "part of the confidence estimation procedure.</p>"

    "<p>In each case, the "
    "estimated FDRs are converted to q-values by sorting the scores and then "
    "taking, for each score, the minimum of the current FDR and all of the FDRs "
    "below it in the ranked list.</p>"

    "<p>If <code>tide-index</code> was used to create multiple decoys per "
    "target using the <code>num-decoys-per-target</code> and the "
    "<code>estimation-method</code> is set to <code>tdc</code>, then <code>"
    "assign-confidence</code> will automatically carry out the average TDC "
    "(aTDC) procedure, which aims to reduce decoy-induced variability in "
    "the FDR estimates produced by TDC. The aTDC procedure is described in</p>"
    "<blockquote>Uri Keich, Kaipo Tamura, and William Stafford Noble. "
    "<a href=\"https://pubs.acs.org/doi/10.1021/acs.jproteome.8b00802\">"
    "\"Averaging strategy to reduce variability in target-decoy "
    "estimates of false discovery rate.\" <em>Journal of Proteome "
    "Research</em>, 18(2):585-593, 2019.</blockquote>"

    "<p>A primer on multiple testing correction "
