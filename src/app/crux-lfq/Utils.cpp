    ...
    const Identification& identification = ms2IdsForThisFile[i];
    {
      ...
      for (const auto& chargeState : chargeStates) {
        ...
        vector<IndexedMassSpectralPeak*> xic = peakFind(
            identification
                .ms2RetentionTimeInMinutes,  // This value may not be in
                                             // minutes, check and convert
...
void quantifyMs2IdentifiedPeptides(
  ...
  std::copy_if(allIdentifications.begin(), allIdentifications.end(),
  ...
  PpmTolerance peakfindingTol(
      PEAK_FINDING_PPM_TOLERANCE);  // Peak finding tolerance is generally
                                    // higher than ppmTolerance
