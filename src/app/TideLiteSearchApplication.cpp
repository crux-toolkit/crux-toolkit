...
int TideLiteSearchApplication::main(const vector<string>& input_files, const string input_index) {
  ...
  for (int t = 1; t < num_threads_; ++t) {
    boost::thread * currthread = new boost::thread(boost::bind(&TideLiteSearchApplication::spectrum_search, this, (void *) &(thread_data_array[t])));
 ...
 for (vector<TideLiteSearchApplication::InputFile>::iterator original_file_name = inputFiles_.begin(); original_file_name != inputFiles_.end(); ++original_file_name) {
    // inputFiles_.push_back(TideLiteSearchApplication::InputFile(*original_file_name, *original_file_name, false));
...
int TideLiteSearchApplication::PeakMatching(ObservedPeakSet& observed, vector<unsigned int>& peak_list, int& matching_peaks, int& repeat_matching_peaks) {
...
int TideLiteSearchApplication::getMaxColEvidence(
...
int TideLiteSearchApplication::calcResEvScore(
  const vector<vector<double> >& curResidueEvidenceMatrix,
...
int TideLiteSearchApplication::calcScoreCount(vector<int>& pepMassIntUnique, vector<vector<int>>& evidenceObs, vector<double>& nullDistribution) {
 ...
 */
void TideLiteSearchApplication::calcResidueScoreCount (
  int pepMassInt,
  ...
  if (curScoreFunction_ == PVALUES) {
    ...
    if (FileUtils::Exists(residue_stats_file)) {  // The amino acid frequency calculation is done in tide-index.
      ...
      while (!(done = residue_stat_reader.Done())) {
        ...
        // printf("aa: %s, \t daamass:%lf'\n", last_residue_stat.aa_str().c_str(), aa_mass);
      ...
      while (!(peptide_reader.Done())) { //read all peptides form index
        ...
        PeptideLite peptide(current_pb_peptide_, proteins);
...
void TideLiteSearchApplication::getInputFiles(int thread_id) {
  ...
  if (thread_id > inputFiles_.size())
    return;
  for (vector<TideLiteSearchApplication::InputFile>::iterator original_file_name = inputFiles_.begin()+thread_id; 
...
void TideLiteSearchApplication::createOutputFiles() {
  ...
  concat_file_name = make_file_path("tide-lite-search.txt");
  target_file_name = make_file_path("tide-lite-search.target.txt");
  decoy_file_name  = make_file_path("tide-lite-search.decoy.txt");
  ...
  if (Params::GetBool("txt-output") == true) {  // original tide-search output format in tab-delimited text files (txt)
    string header = TideLiteMatchSet::getHeader(TIDE_SEARCH_TSV, tide_index_mzTab_file_path_);
  ...
  concat_file_name = make_file_path("tide-lite-search.mzTab");
  target_file_name = make_file_path("tide-lite-search.target.mzTab");
  decoy_file_name  = make_file_path("tide-lite-search.decoy.mzTab");
  ...
  if ( Params::GetBool("mztab-output") == true) {   // mzTAB output format
    ...
    // Get column headers
    string header = TideLiteMatchSet::getHeader(TIDE_SEARCH_MZTAB_TSV, tide_index_mzTab_file_path_);  // Gets the column headers
    if (concat) {
...
void TideLiteSearchApplication::PrintResults(const SpectrumCollection::SpecCharge* sc, string spectrum_file_name, int spectrum_file_cnt, TideLiteMatchSet* psm_scores) {
...
double TideLiteSearchApplication::calcCombinedPval(
  double m, //parameter
