...
void Params::Initialize() {
  ...
  InitStringParam("protein-database", "",
    ...
    "Argument for generate. Optional for analyze and spectral-counts.", false);
  InitStringParam("search results directory", "",
    "Directory containing the results of one search.",
    "Argument for q-ranker, percolator, compute-q-values.", false);
  InitStringParam("protein fasta file", "",
   "File containing protein sequences in fasta format.",
   "Argument for tide-index and generate-decoys.", false);
  InitStringParam("index name", "",
    "Name to give the new directory containing index files.",
    "Argument for tide-index.", false);
  InitStringParam("ms2 file", "",
    "File containing spectra to be searched.",
    "Argument, not option, for create-psm-files and get-ms2-spec", false);
  /* get-ms2-spectrum */
  ...
  InitStringParam("peptide sequence", "", 
    "The sequence of the peptide.",
    "Argument for predict-peptide-ions.", false);
  InitIntParam("charge state", 0, 0, 10, 
    "The charge state of the peptide.",
    "Argument for predict-peptide-ions", false);
  /* hardklor arguments */
  InitStringParam("spectra", "",
    ...
    "(.mzXML) format.",
    "Argument for hardklor, q-ranker, and barista", false);
  ...
  InitStringParam("pin", "",
    ...
    "to execution.",
    "Argument, not option for percolator", false);
  /*make-pin arguments*/
  InitStringParam("target input", "",
    ...
    "standard input",
    "Argument for make-pin and calibrate-scores", false);
  ...
  InitBoolParam("original-output", false,
    ...
    "Available for crux percolator.", false);
  // **** Tide arguments ****
  InitStringParam("spectrum records file", "",
    "A spectrum records file generated by a previous run of crux tide-search "
    "using the store-spectra parameter.",
    "Available for read-spectrumrecords", "true");
  InitStringParam("tide spectra file", "",
    ...
    "Available for tide-search", "true");
  InitStringParam("tide database index", "",
  ...
  InitStringParam("search results", "",
...
void Params::AddArgValue(const string& name, const string& value) {
  container_.Add(new ArgParam(name, value));
}
...
void Params::ParamContainer::Add(Param* param) {
  ...
  if (!param->IsArgument()) {
    ...
    Param* existing = GetParam(paramName);
    if (existing->IsArgument()) {
      ((ArgParam*)existing)->AddValue(param->GetString());
    } else {
      // Existing param is not an ArgParam, delete it
      // TODO Warn? Handle better?
      paramsOrdered_.erase(remove(paramsOrdered_.begin(), paramsOrdered_.end(), existing),
                           paramsOrdered_.end());
      delete existing;
      params_[paramName] = param;
    }
  }
}
...
string Param::GetParamFileString() const {
  vector<string> lines = StringUtils::Split(usage_, '\n');
  vector<string> noteLines = StringUtils::Split(fileNotes_, '\n');
...
// ArgParam
//
ArgParam::ArgParam(const string& name, const string& value)
  : Param(name, "", "", false), values_(vector<string>(1, value)) {}
