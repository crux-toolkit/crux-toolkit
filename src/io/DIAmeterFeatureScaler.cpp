...
void DIAmeterFeatureScaler::writeScaledFile(const char* output_file_name) {
	 ...
	 *output_file << StringUtils::Join(column_names, '\t').c_str() << endl;
	 ...
	 while (fileReader_->hasNext()) {
		 output_vec.clear();
		 for (int column_idx = 0; column_idx < column_names.size(); column_idx++) { output_vec.push_back(fileReader_->getString(column_idx)); }
		 ...
		 for (int idx = 0; idx < toscale_column_indices_.size(); idx++) {
			 ...
			 // carp(CARP_DEBUG, "old_score:%f \t new_score:%f \t str_score:%s", old_score, new_score, output_vec.at(curr_column_idx).c_str() );
		 }
...
void DIAmeterFeatureScaler::calcDataQuantile(double quantile_low, double quantile_high) {
	...
	carp(CARP_DEBUG, "Record:%d ", record_cnt );
	for (int idx = 0; idx < toscale_column_indices_.size(); idx++) {
		int curr_column_idx = toscale_column_indices_.at(idx);
		...
		int quantile_low_pos = (int)(quantile_low*(double)curr_match_values->size()+0.5);
		...
		carp(CARP_DEBUG, "ColumnIndex:%d \t ColumnName:%s \t Size:%d \t quantile_low:%f \t quantile_high:%f", curr_column_idx, get_column_header(toscale_column_ids_.at(idx)), toscale_match_values_[curr_column_idx]->size(), quantile_low_score, quantile_high_score );
	}
}
...
void DIAmeterFeatureScaler::parseHeader() {
  ...
  for (int idx = 0; idx < sizeof(toscale_columns_)/sizeof(toscale_columns_[0]); idx++) {
	  ...
	  carp(CARP_DEBUG, "ColumnID:%d \t ColumnIndex:%d \t ColumnName:%s", curr_column_id, curr_column_idx, curr_column_name );
